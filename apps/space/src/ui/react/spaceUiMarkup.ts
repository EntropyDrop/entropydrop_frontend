// Trusted, build-time UI markup migrated verbatim from apps/space/index.html.
// React owns this tree; UIManager is the engine adapter for its stable IDs.
export const spaceUiMarkup = `<!-- Center Crosshair -->
  ...
  <div id="hud-overlay">
    ...
    <div class="hud-top">
      <div class="hud-card">
        <div class="hud-badge"><span class="hud-badge-dot"></span>EntropyDrop · Space</div>
        <div class="hud-metrics-row">
          <span id="fps-val">60 FPS</span>
          <span class="hud-metric-sep">·</span>
          <span id="ping-val" class="hud-ping ping-unknown">-- ms</span>
        </div>
        <div id="pos-val">X: -- | Y: -- | Z: --</div>

        <!-- Nearby Entities Collapsible Section -->
        <div class="hud-entities-section" id="hud-entities-section">
          <div class="hud-entities-header" id="hud-entities-toggle" role="button" tabindex="0" title="Toggle nearby entities list">
            <div class="hud-entities-title">
              <span class="hud-entities-icon">⬡</span>
              <span>Nearby Entities (<span id="hud-entities-count">0</span>)</span>
            </div>
            <button type="button" id="hud-entities-toggle-btn" class="hud-entities-toggle-btn" aria-label="Toggle entities list">▼</button>
          </div>
          <div class="hud-entities-body" id="hud-entities-body" style="display: none;">
            <div class="hud-entities-list" id="hud-entities-list">
              <!-- Dynamically populated -->
            </div>
            <div class="hud-entities-pagination" id="hud-entities-pagination" style="display: none;">
              <button type="button" id="hud-entities-prev-btn" class="hud-page-btn" disabled title="Previous page">◀</button>
              <span id="hud-entities-page-info" class="hud-page-info">1 / 1</span>
              <button type="button" id="hud-entities-next-btn" class="hud-page-btn" disabled title="Next page">▶</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="hud-actions">
        <button id="global-settings-btn" class="icon-btn" title="Global Settings (O)">⚙</button>
      </div>
    </div>

    <!-- Bottom Bar: Tools Hotbar (Slots 1-6) + Dynamic Tool Panel (Palette / Backpack / Selector) -->
    <div class="hud-bottom">
      <div class="builder-toolbar">
        <div class="toolbar-center-panel">
          <div class="color-palette-bar-wrapper" id="color-palette-wrapper">
            <div class="palette-info-row">
              <span class="palette-title">Palette</span>
              <span class="palette-hotkey-hint"><b>Shift+1~9</b> pick · <b>E</b> set colors</span>
            </div>
            <div id="color-palette-bar" class="color-palette-bar"></div>
          </div>
          <div class="inventory-bar-wrapper" id="inventory-bar-wrapper" style="display: none;">
            <div class="palette-info-row">
              <div class="palette-title-group">
                <span class="palette-title" id="backpack-bar-title" style="cursor: pointer;" title="Click or press E to open full backpack">Backpack</span>
                <div id="inv-cat-tabs" class="inv-cat-tabs"></div>
              </div>
              <span class="palette-hotkey-hint"><b>E</b> Full Backpack · <b>Tab</b> BKS↔ENT</span>
            </div>
            <div id="inventory-bar" class="inventory-bar"></div>
          </div>
          <div class="selector-panel-wrapper" id="selector-panel-wrapper" style="display: none;">
            <div class="palette-info-row">
              <div class="selector-title-group">
                <span class="palette-title">Selector</span>
                <button id="selector-mode-toggle" class="selector-mode-btn" title="Click or press Tab to switch mode">
                  <span id="selector-mode-badge" class="mode-badge std">STANDARD</span>
                  <span class="mode-tab-hint">Tab ⇋</span>
                </button>
              </div>
              <span class="palette-hotkey-hint"><b>Tab</b> mode · <b>R</b> copy · <b>G</b> assemble</span>
            </div>
            <div class="selector-toolbox-content" id="selector-toolbox-content">
              <div class="selector-status-col">
                <div class="selector-status-main">
                  <span class="selector-icon">⬡</span>
                  <span id="selector-panel-title" class="selector-status-title">Selector</span>
                </div>
                <div id="selector-panel-details" class="selector-status-sub"></div>
              </div>
              <div class="selector-action-buttons">
                <button id="assemble-btn" class="banner-btn primary" disabled>Assemble (G)</button>
                <button id="copy-btn" class="banner-btn secondary" title="Copy selection to backpack (R)" disabled>Copy (R)</button>
              </div>
            </div>
          </div>
          <div id="hotbar"></div>
  ...
  <!-- Entity Agent + Programmable Controller Modal (C key) -->
  <div id="code-editor-modal" class="custom-modal">
    <div class="modal-content code-editor-container">
      <!-- Editor Header -->
      <div class="editor-header">
        <div class="editor-title-group">
          <div class="editor-title">Entity Editor</div>
          <div id="editor-entity-id" class="editor-tag" title="Click to copy Entity ID">ID: ent_pending</div>
          <div id="editor-status-badge" class="status-badge running">RUNNING</div>
          <div id="editor-exec-time" class="exec-time">0.05 ms</div>
        </div>

        <div class="editor-actions">
          <div class="pb-radio-group" id="global-playback-group" title="Global script control">
            <input type="radio" id="pb-global-play" name="pb-global" value="play">
            <label for="pb-global-play" class="pb-option play" title="Play: enable all component scripts">▶</label>
            <input type="radio" id="pb-global-pause" name="pb-global" value="pause">
            <label for="pb-global-pause" class="pb-option pause" title="Pause: disable all component scripts">⏸</label>
            <input type="radio" id="pb-global-stop" name="pb-global" value="stop">
            <label for="pb-global-stop" class="pb-option stop" title="Stop: disable all scripts and reset state, clock, transforms, and forces">⏹</label>
          </div>
          <button id="run-script-btn" class="editor-btn run-btn">Apply Code</button>
          <button id="api-docs-btn" class="editor-btn" title="Open the script API reference (documentation)">📖 Docs</button>
          <button id="close-code-btn" class="icon-btn" style="width: 28px; height: 28px; font-size: 13px;" title="Close terminal (ESC)">✕</button>
        </div>
      </div>
      ...
      <div class="editor-body">
        ...
        <div class="hierarchy-sidebar">
          ...
          <div id="component-tree-panel" class="component-tree-panel">
            <div id="component-tree-list" class="component-tree-list"></div>
          </div>

          <!-- Component Property Inspector -->
          <div class="sidebar-section-title" style="margin-top: 8px;">
            <span>COMPONENT INSPECTOR</span>
            <span id="component-inspector-id" class="component-inspector-badge">root</span>
          </div>
          <div id="component-inspector-panel" class="component-inspector-panel">
            <div class="inspector-field">
              <label class="inspector-label">ID</label>
              <div class="inspector-input-row">
                <input id="prop-node-name" class="inspector-input" type="text" value="root">
                <button id="prop-rename-btn" class="small-action-btn" title="Rename component id (unique across the whole entity)">Rename</button>
              </div>
            </div>
            <div class="inspector-field has-tooltip">
              <label class="inspector-label" title="Component role in the hierarchy: root body is the main rigid body, child is an attached sub-assembly">Type ⓘ</label>
              <span id="prop-node-kind" class="inspector-val">root body</span>
              <div class="tooltip-text">Role in hierarchy:<br><b>root body</b> is the entity's main rigid body;<br><b>child</b> is an attached sub-assembly.</div>
            </div>
            <div class="inspector-field">
              <label class="inspector-label">Parent</label>
              <span id="prop-node-parent" class="inspector-val">None</span>
            </div>
            <div class="inspector-grid inspector-grid-three">
              <div class="inspector-field has-tooltip">
                <label class="inspector-sublabel" for="prop-body-type" title="Rigid body mode: kinematic is script-driven without gravity/forces; dynamic simulates full Newtonian physics">Rigid Body ⓘ</label>
                <select id="prop-body-type" class="inspector-input">
                  <option value="kinematic">kinematic</option>
                  <option value="dynamic">dynamic</option>
                </select>
                <div class="tooltip-text"><b>kinematic</b>: Moves strictly via script, immune to gravity and forces.<br><b>dynamic</b>: Full physics simulation (gravity, thrust, collisions).</div>
              </div>
              <div class="inspector-field has-tooltip">
                <label class="inspector-sublabel" for="prop-restitution" title="Coefficient of restitution (0.0–1.0): collision bounciness (0 = no bounce, 1 = perfectly elastic)">Restitution ⓘ</label>
                <input id="prop-restitution" class="inspector-input" type="number" min="0" max="1" step="0.01" value="0.01">
                <div class="tooltip-text">Collision bounciness (0.0 to 1.0).<br>0.0 is inelastic (no bounce), 1.0 is perfectly elastic bounce.</div>
              </div>
              <div class="inspector-field has-tooltip">
                <label class="inspector-sublabel" for="prop-mass" title="Rigid body mass (kg): determines inertia and acceleration (F = m · a). Defaults to volume × 10 kg/m³">Mass (kg) ⓘ</label>
                <input id="prop-mass" class="inspector-input" type="number" min="0.1" step="1" value="10">
                <div class="tooltip-text">Mass in kg, determining inertia & acceleration (F = m · a).<br>Defaults to voxel volume × 10 kg/m³ (micro block = 0.08 kg, min 0.1 kg).</div>
              </div>
            </div>
            <div class="inspector-grid">
              <div class="inspector-item">
                <span class="inspector-sublabel">Blocks</span>
                <span id="prop-node-blocks" class="inspector-num">12</span>
              </div>
              <div class="inspector-item">
                <span class="inspector-sublabel">Volume</span>
                <span id="prop-node-volume" class="inspector-num">12.0 m³</span>
              </div>
            </div>
            <div class="inspector-field has-tooltip">
              <label class="inspector-label" title="Joints & constraints connecting this component to its parent or child nodes">Constraints ⓘ</label>
              <span id="prop-node-constraints" class="inspector-val">0</span>
              <div class="tooltip-text">Physical joints/constraints connecting this component to parent or child nodes (welds, hinges, sliders).</div>
            </div>
            <div class="inspector-field has-tooltip">
              <label class="inspector-label" title="Local anchor center [x, y, z] (m) for rotation center, force applications, and physics calculations">Pivot ⓘ</label>
              <span id="prop-node-pivot" class="inspector-val mono">[0.0, 0.0, 0.0]</span>
              <div class="tooltip-text">Local anchor point [x, y, z] (m) for rotation center, physics calculations, and force applications.</div>
            </div>
            <div class="inspector-field has-tooltip">
              <label class="inspector-label" title="3D position offset [x, y, z] (m) relative to the parent coordinate frame">Local Pos ⓘ</label>
              <span id="prop-node-pos" class="inspector-val mono">[0.0, 0.0, 0.0]</span>
              <div class="tooltip-text">3D position offset [x, y, z] (m) relative to the parent component's local coordinate frame.</div>
            </div>
            <div class="inspector-field has-tooltip">
              <label class="inspector-label" title="Euler rotation angles [pitch°, yaw°, roll°] relative to the parent coordinate frame">Local Rot ⓘ</label>
              <span id="prop-node-rot" class="inspector-val mono">[0.0, 0.0, 0.0]</span>
              <div class="tooltip-text">Euler orientation angles [pitch°, yaw°, roll°] relative to the parent component's coordinate frame.</div>
            </div>
          </div>
        </div>
        ...
        <div class="code-area-wrapper">
          <div class="code-tab-bar" id="code-tab-bar">
            <!-- Dynamic component tabs, e.g. [R root] [C child_1] -->
          </div>
          <div class="code-editor-main">
            <div class="code-gutter" id="code-gutter"></div>
            <textarea id="script-textarea" class="code-textarea" spellcheck="false" placeholder="// Write your controller code here..."></textarea>
          </div>
          <div class="code-footer-hint" id="code-footer-hint">
            <span id="code-target-hint">Editing: root (body)</span>
            <span id="code-api-hint" class="code-api-hint">API: self · ctx</span>
          </div>
        </div>

        <!-- Right: 3D Preview, Direct state API telemetry, Console Logs -->
        <div class="telemetry-panel">
          <div class="telemetry-section-title">3D VIEW</div>
          <div class="entity-preview-frame">
            <canvas id="entity-preview-canvas" aria-label="Interactive preview of the entity in the current world"></canvas>
          </div>

          <div class="telemetry-section-title">ENTITY STATE API</div>

          <div class="telemetry-grid">
            <div class="telemetry-item">
              <span class="tele-label">Ground Dist</span>
              <span id="tele-ground-dist" class="tele-val highlight">4.50 m</span>
            </div>
            <div class="telemetry-item">
              <span class="tele-label">Altitude</span>
              <span id="tele-altitude" class="tele-val">18.5 m</span>
            </div>
            <div class="telemetry-item">
              <span class="tele-label">Speed</span>
              <span id="tele-speed" class="tele-val">0.00 m/s</span>
            </div>
            <div class="telemetry-item">
              <span class="tele-label">Mass</span>
              <span id="tele-mass" class="tele-val">14.4 kg</span>
            </div>
            <div class="telemetry-item power-item has-tooltip">
              <span class="tele-label" title="Percentage of maximum force and torque budget currently consumed by legacy top-level root force commands">Root Power Budget ⓘ</span>
              <span id="tele-power" class="tele-val highlight">0%</span>
              <span class="power-meter"><span id="tele-power-fill"></span></span>
              <div class="tooltip-text"><b>Root Power Budget</b>: % of max force & torque budget used by legacy top-level root commands (<code>applyForce</code>, <code>applyTorque</code>). Per-component <code>self.body.apply*</code> is unmetered.</div>
            </div>
          </div>

          <div class="telemetry-section-title" style="margin-top: 8px;">CONSOLE LOGS</div>
          <div id="tele-console-logs" class="console-logs-box">
            <div class="log-line text-muted">Terminal ready, waiting for script output...</div>
          </div>
        </div>

        <!-- Far right: AI Assistant Chat column -->
        <div class="agent-column">
          <div class="telemetry-section-title agent-chat-title">
            <span>AI ASSISTANT</span>
            <div class="agent-title-actions">
              <button id="agent-clear-btn" class="mini-toggle-btn reset" title="Clear chat history">🗑 CLEAR</button>
              <button id="agent-settings-btn" class="mini-toggle-btn reset" title="Toggle model API settings">
                ⚙ SETUP <span id="agent-setup-arrow" class="setup-arrow">▼</span>
              </button>
            </div>
          </div>

          <!-- Accordion Collapsible Setup Panel -->
          <div id="agent-setup-accordion" class="agent-setup-accordion" style="display: none;">
            <div class="agent-config-field">
              <span class="config-label">API Base URL</span>
              <input id="agent-api-base" class="config-input" type="text" placeholder="https://api.openai.com/v1" autocomplete="off">
            </div>
            <div class="agent-config-field">
              <span class="config-label">API Key</span>
              <input id="agent-api-key" class="config-input" type="password" placeholder="sk-..." autocomplete="off">
            </div>
            <div class="agent-config-field">
              <span class="config-label">Model</span>
              <input id="agent-api-model" class="config-input" type="text" placeholder="gpt-4o-mini" autocomplete="off">
            </div>
            <div class="agent-config-field">
              <span class="config-label">Context Window (K tokens)</span>
              <input id="agent-context-length" class="config-input" type="number" min="1" max="2048" step="1" placeholder="32" autocomplete="off">
            </div>
            <div class="agent-config-field">
              <span class="config-label">Max Output (K tokens)</span>
              <input id="agent-max-tokens" class="config-input" type="number" min="0.1" max="128" step="0.5" placeholder="4" autocomplete="off">
            </div>
            <div class="agent-config-actions">
              <button id="agent-config-save-btn" class="small-btn primary">Save Config</button>
            </div>
            <div class="config-hint">Key is saved in browser localStorage. Without a key, uses local compiler.</div>
          </div>

          <div id="agent-chat-box" class="agent-chat-box">
            <div class="agent-chat-msg agent-msg-system">Tip: describe a behavior in plain language (e.g. &quot;hover 5 meters above ground&quot;). Configure an API Key to use a real model, otherwise the built-in local compiler is used.</div>
          </div>
          <div class="agent-chat-input-row">
            <input id="agent-chat-input" class="agent-chat-input" type="text" autocomplete="off"
              placeholder="e.g. follow me 3m behind to the right..." />
            <button id="agent-chat-send-btn" class="agent-send-btn">Send</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- Structure Blueprints Modal (B key) -->
  <div id="blueprints-modal" class="custom-modal">
    <div class="modal-content large">
      <div class="modal-header">
        <h2>Structure Blueprints</h2>
        <button id="close-blueprints-btn" class="icon-btn" style="width: 28px; height: 28px; font-size: 13px;">✕</button>
      </div>
      <div class="modal-sub">Click to spawn a colored structure in front of you — ready to use!</div>
      <div id="blueprints-grid" class="blueprints-grid"></div>
    </div>
  </div>

  <!-- Tools & Colors Inventory Modal (E key) -->
  <div id="inventory-modal" class="custom-modal">
    <div class="modal-content">
      <div class="modal-header">
        <h2>Backpack</h2>
        <button id="close-inv-btn" class="icon-btn" style="width: 28px; height: 28px; font-size: 13px;">✕</button>
      </div>
      <div class="modal-sub">9 block sets · 9 entities · 9 color sets — exportable as JSON · ⬡ STL meshes importable as block sets</div>
      <div id="inventory-grid" class="inventory-grid"></div>
    </div>
  </div>

  <!-- Script API Reference Modal (button inside the code editor) -->
  <div id="api-docs-modal" class="custom-modal">
    <div class="modal-content api-docs-container">
      <div class="modal-header">
        <h2>📖 ENTITY SCRIPT API V2 REFERENCE</h2>
        <button id="close-api-docs-btn" class="icon-btn" style="width: 28px; height: 28px; font-size: 13px;" title="Close docs (ESC)">✕</button>
      </div>
      <div class="modal-sub">Entity behavior script reference · isolated QuickJS per loaded entity · one script per component · press C to open the editor</div>
      <div class="api-docs-body" id="api-docs-body">
        ...
        <div class="api-section">
          ...
          <table class="api-table">
            ...
            <tbody>
              ...
              <tr><td><code>self.body.getMaterial()</code> / <code>self.body.setMaterial({...})</code></td><td>Read <code>{restitution,friction}</code>; setter returns <code>{ok,material,reason}</code>. Both coefficients clamp to <code>[0,1]</code>. Restitution defaults to <code>0.01</code> and friction to <code>0.7</code></td></tr>
...
// if (ctx.blocks.pressed('subdivide')) { ... } // 1x1x1 → 5x5x5 carving</code></pre>
        ...
        <div class="api-section">
          ...
          <ul class="api-ul">
            ...
            <li>Use the Selector to select an entity and press <b>R</b> to copy it. Copying automatically switches to Hammer, which previews and builds the inventory item with left-click. Use Wrench left-click on an entity to start/pause its code, or right-click to stop and reset runtime state. The blueprint library (<b>B</b>) contains ready-made drone, windmill, and Ferris wheel examples.</li>
          </ul>
        </div>
  ...
  <!-- Global Settings Modal (O key or top-right gear icon) -->
  <div id="global-settings-modal" class="custom-modal">
    <div class="modal-content settings-modal-content">
      <div class="modal-header">
        <h2>Global Settings</h2>
        <button id="close-global-settings-btn" class="icon-btn" style="width: 28px; height: 28px; font-size: 13px;" title="Close settings (ESC)">✕</button>
      </div>
      <div class="modal-sub">Configure camera, perspective, and world preferences</div>

      <div class="settings-section">
        <div class="settings-section-title">CAMERA &amp; VIEW</div>

        <!-- Camera FOV -->
        <div class="settings-row">
          <div class="settings-label-group">
            <span class="settings-label">Field of View (FOV)</span>
            <span class="settings-desc">Camera lens angle (50° ~ 110°)</span>
          </div>
          <div class="settings-control-group">
            <input id="setting-fov-slider" class="settings-slider" type="range" min="50" max="110" step="1" value="75">
            <span id="setting-fov-val" class="settings-value-badge">75°</span>
          </div>
        </div>
        ...
        <div class="settings-row">
          <div class="settings-label-group">
            <span class="settings-label">Perspective</span>
            <span class="settings-desc">Cycle First, Third Back, and Third Front views (F3)</span>
          </div>
          <div class="settings-segmented-control" id="setting-perspective-group">
            <button class="segment-btn active" data-perspective="first_person">1st Person</button>
            <button class="segment-btn" data-perspective="third_person">3rd Back</button>
            <button class="segment-btn" data-perspective="third_person_front">3rd Front</button>
          </div>
        </div>

        <!-- Third Person Distance -->
        <div class="settings-row" id="setting-cam-dist-row">
          <div class="settings-label-group">
            <span class="settings-label">Third Person Distance</span>
            <span class="settings-desc">Camera offset distance from player</span>
          </div>
          <div class="settings-control-group">
            <input id="setting-cam-dist-slider" class="settings-slider" type="range" min="2" max="8" step="0.5" value="4">
            <span id="setting-cam-dist-val" class="settings-value-badge">4.0 m</span>
          </div>
        </div>
      </div>

      <div class="settings-section">
        <div class="settings-section-title">PHYSICS &amp; ENVIRONMENT</div>
        <div class="settings-row">
          <div class="settings-label-group">
            <span class="settings-label">Entity Gravity</span>
            <span class="settings-desc">Rigid body world gravity simulation</span>
          </div>
          <div class="settings-segmented-control" id="setting-gravity-group">
            <button class="segment-btn active" data-gravity="-18">Standard (-18)</button>
            <button class="segment-btn" data-gravity="-5">Moon (-5)</button>
            <button class="segment-btn" data-gravity="0">Zero-G (0)</button>
          </div>
        </div>
        ...
        <div class="settings-row">
          <div class="settings-label-group">
            <span class="settings-label">Chunk Render Distance</span>
            <span class="settings-desc">Voxel terrain mesh streaming radius (4 ~ 20 chunks)</span>
          </div>
          <div class="settings-control-group">
            <input id="setting-render-dist-slider" class="settings-slider" type="range" min="4" max="20" step="1" value="12">
            <span id="setting-render-dist-val" class="settings-value-badge">12 Chunks</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- Start / Pause Hero Overlay -->
  <div id="pause-screen">
    <div class="hero-box">
      <div class="hero-gear-icon">✦</div>
      <h1 class="game-logo">EntropyDrop · Space</h1>
      <p class="game-sub">Build anything. Tell it what to do. · AI-native programmable voxel physics</p>

      <div class="features-highlight">
        <div class="feat-item">
          <span class="feat-icon">✦</span>
          <b>Behavior via Natural Language</b>
          <p>Point at an entity and press C — say &quot;follow me&quot; or &quot;hover 5 meters&quot; to generate code</p>
        </div>
        <div class="feat-item">
          <span class="feat-icon">⬡</span>
          <b>One Block · Any Color</b>
          <p>Material type never limits your shape; instant color with palette, brush, and pipette</p>
        </div>
        <div class="feat-item">
          <span class="feat-icon">⚡</span>
          <b>Dual-Scale Micro-Carving</b>
          <p>The shovel edits standard 1m blocks; the spoon carves each into 5x5x5 micro voxels</p>
        </div>
      </div>

      <div class="controls-guide">
        <span><kbd class="key-badge">W</kbd><kbd class="key-badge">A</kbd><kbd class="key-badge">S</kbd><kbd class="key-badge">D</kbd> Move / Drive</span>
        <span><kbd class="key-badge">Space</kbd> Jump / Ascend</span>
        <span><kbd class="key-badge">Shift+1-9</kbd> Palette color / Backpack slot</span>
        <span><kbd class="key-badge">E</kbd> Backpack / Set colors</span>
        <span><kbd class="key-badge">1</kbd> Shovel: remove / place 1m blocks</span>
        <span><kbd class="key-badge">2</kbd> Spoon: micro-carve 5x5x5</span>
        <span><kbd class="key-badge">3</kbd> Brush: paint / right-click sample</span>
        <span><kbd class="key-badge">MMB</kbd> Sample color anywhere</span>
        <span><kbd class="key-badge">4</kbd> Selector: box select (max 64×64×64) · Tab standard/micro blocks · R copy</span>
        <span><kbd class="key-badge">5</kbd> Hammer: preview &amp; LMB build / RMB overwrite</span>
        <span><kbd class="key-badge">6</kbd> Wrench: LMB drag force · RMB start/stop</span>
        <span><kbd class="key-badge">Shift+Click</kbd> Multi-select component blocks</span>
        <span><kbd class="key-badge">C</kbd> Entity editor</span>
        <span><kbd class="key-badge">G</kbd> Assemble physics entity</span>
        <span><kbd class="key-badge">V</kbd> Mount / leave drivable entity</span>
        <span><kbd class="key-badge">B</kbd> Structure blueprint library</span>
        <span><kbd class="key-badge">F</kbd> Fly mode</span>
        <span><kbd class="key-badge">F3</kbd> Cycle 1st / 3rd Back / 3rd Front</span>
        <span><kbd class="key-badge">ESC</kbd> Settings / release cursor</span>
      </div>

      <button id="start-btn" class="start-btn">Enter Space</button>
    </div>
  </div>

<!-- React-owned hosts for canvas/navigation UI adapters -->
<div id="minimap-container" class="minimap-container"><canvas class="minimap-canvas"></canvas></div>
<div id="nav-system-container" class="nav-system-container">
  <div class="nav-badge"><span class="nav-badge-dot"></span><span>AUTO PILOT</span></div>
  <div class="nav-coord-inputs">
    <div class="nav-input-field"><span class="nav-coord-label">X</span><input type="number" id="nav-input-x" class="nav-number-input" placeholder="0" step="any"></div>
    <div class="nav-input-field"><span class="nav-coord-label">Y</span><input type="number" id="nav-input-y" class="nav-number-input" placeholder="20" step="any"></div>
    <div class="nav-input-field"><span class="nav-coord-label">Z</span><input type="number" id="nav-input-z" class="nav-number-input" placeholder="0" step="any"></div>
  </div>

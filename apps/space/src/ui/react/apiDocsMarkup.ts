...
const dy = playerPos[1] - ctx.position[1]; // no wrap on Y</code></pre>
        ...
        <div class="api-section">
          ...
          <ul class="api-ul">
            ...
            <li>Read state from <code>ctx</code>. Dynamic bodies move through force/torque APIs and cannot accept direct pose writes; kinematic bodies additionally accept the documented <code>setLocalPosition</code>/<code>setLocalRotation</code>/<code>setLocalEuler</code> commands. A collision-enabled kinematic pose is clipped rather than allowed to pass through another entity, but receives no force impulse. No public API directly sets velocity.</li>

...
## Model and lifecycle
- Every world entity has a stable random ctx.entityId (ent_<UUID>) and is a component tree. Every component script has the same signature (self, ctx), is submitted once per physics frame (~60 Hz), and owns persistent self.state. Browser execution is pipelined through a Worker, so commands normally commit on the following physics frame. self is the component named in the current Target component note; ctx.position/velocity/rotation/mass/bodyType always describe the root entity.
...
- Execution: one shared Worker hosts isolated QuickJS execution and every entity owns a separate 4 MiB Runtime. The root script runs before child scripts; all components of the entity share one frozen ctx snapshot per script frame. Admitted mutations return provisional reason 'queued', then are revalidated and committed on the main thread; a full 256-command entity frame buffer returns ok:false with reason 'command_limit'. Standard-world writes have an in-tick overlay; world.entities filters a nearby 64 m snapshot. Full synchronous world voxel/micro getters and raycast are unavailable across the Worker boundary (unwritten voxel getters return air; raycast returns null).
...
## Component API (self)
- Identity/tree/state: self.id, self.parentId, self.state, self.child(id), self.children().
...
- Legacy root-body forces: applyForce(worldForce), applyLocalForce(localForce), applyThrust(rootLocalForceAtThisComponent), applyForceAt(worldForce, componentLocalPoint), applyTorque(worldTorque). They have no effect on a kinematic root. Commands are per-frame and clamped by ctx.limits (maxForce = max(80, mass*65); maxTorque = max(40, maxForce*max(0.75, boundingRadius))). The ctx.limits clamp and HUD Power Budget cover only this legacy root-body force surface; self.body.apply* is not clamped or included in that meter.
- Root-only: setCockpitPosition/getCockpitPosition (default [0,0,0]), setVehicle/getVehicle (default true), self.stop(). self.stop() is identical to the global Stop button: it disables every component script and resets component state, script time/tick, child transforms/spins, and pending forces/torques. A child must call ctx.root.stop().

## Frame context (read-only)
ctx.entityId, ctx.root, time, deltaTime, tick, position, velocity, rotation[pitch,yaw,roll], angularVelocity, groundDistance, mass, bodyType, gravity, limits{maxForce,maxTorque} for the legacy root-body force surface, input, blocks, players, world, selection, log(msg).
- input.down(code), pressed(code), released(code); codes include KeyW/A/S/D, Space and ShiftLeft; Shift/Control/Alt match either side. Only the mounted entity receives player input. Reserved keys never reach scripts: Escape, Backspace, Delete, F3, F5, KeyB, KeyC, KeyE, KeyF, KeyG, KeyR, KeyT, KeyV, Digit0, Digit1, Digit2, Digit3, Digit4, Digit5, Digit6, Digit7, Digit8, Digit9.
...
12. Gate destructive selection commands with self.state or an input edge; never call delete/assemble/createChild unconditionally every frame.

...
```text
entropydrop_website/
  entropydrop_frontend/        # main site + apps/space (this app)
  entropydrop_space_engine/    # shared TypeScript engine + proto contracts
  entropydrop_backend/         # FastAPI Space service + hosting worker
```

## Responsibility split

- **`entropydrop_space_engine`** owns voxel/chunk data, terrain generation, meshing, torus
  ...
  (`src/storage/InventoryProtobuf.ts`) and the `proto/` schemas. It does not import either
  application.
- **`entropydrop_frontend/apps/space`** owns the browser experience: React UI, Three.js
...
The Vite config builds `apps/space` with `base: '/space/app/'`. The frontend root's

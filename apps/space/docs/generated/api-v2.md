...
| `ctx.players` | array | Frozen `[{id,position,mass}]` list. Position is the player eye (feet + 1.62 m standing or + 1.3 m crouched), not feet; mass is fixed at 50 kg. |
...
| `ctx.world.voxels.get(position)` | Read only the current tick's standard-write overlay; otherwise returns air. |
...
| `ctx.world.microVoxels.get(cell, offset)` | Full synchronous micro-world reads are unavailable; currently returns air. |
...
| `ctx.world.entities(origin, radius=16)` | Filter the prefetched 64 m nearby-entity snapshot using shortest wrapped X/Z distance. |
...
| `ctx.world.raycast(origin, direction, maxDistance=24)` | Bounded synchronous standard-voxel raycast; returns `{block,color,normal,position,distance}` or `null`; maximum 64 calls per entity tick. |

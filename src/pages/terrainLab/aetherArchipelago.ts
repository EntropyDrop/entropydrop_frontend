/** An unbounded, coordinate-addressed archipelago, not a repeated island prefab.
 * All exported geometry uses 1m / 0.125m cubes and normal / emissive materials.
 * X/Z denote cube centres relative to the crop; Y denotes the bottom. */
...
export function generateAetherArchipelago(config: AetherConfig) {
  ...
  const voxels: AetherVoxel[] = [], micros = new Map<number, AetherVoxel>();
  ...
  const micro = (x: number, y: number, z: number, color: number, intensity = 0) => {
    ...
    micros.set(key, { x: x + M / 2 - ox - width / 2, y, z: z + M / 2 - oz - depth / 2,
      sizeX: M, sizeY: M, sizeZ: M, color: architecturalColors?.get(color) ?? color,
  ...
  const castle = (island: AetherIsland) => {
    ...
    for (let v = -2.5; v <= 2.5; v += M) for (let u = -2.5; u <= 2.5; u += M) {
  ...
  for (let y = 0; y < ceiling; y++) for (let z = 0; z < depth; z++) for (let x = 0; x < width; x++) {
    ...
    const { color, emission } = palette[id];
    voxels.push({ x: x + 0.5 - width / 2, y, z: z + 0.5 - depth / 2, sizeX: 1, sizeY: 1, sizeZ: 1,
      color, material: emission ? 'emissive' : 'normal', emissive: emission > 0, intensity: emission });
    standardCount++; if (emission) emissiveCount++;
  ...
  for (const v of micros.values()) {
    ...
    voxels.push(v); if (v.emissive) emissiveCount++;
    heights[x + z * width] = Math.max(heights[x + z * width], v.y + M);
  }
  return { voxels, heights, width, depth, standardCount, emissiveCount, islands: visibleIslands, castles, bridges, waterfalls };
}

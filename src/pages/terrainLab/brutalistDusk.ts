 ...
 * X/Z are cube centres in the preview, Y is the cube bottom in metres. */
...
export interface ConcreteVoxel {
...
interface Viaduct { x: number; z: number; length: number; level: number; alongX: boolean; supports: number[] }
...
const LETTERS: Record<string, string[]> = {
  'П': ['1111', '1001', '1001', '1001', '1001'], 'Р': ['1110', '1001', '1110', '1000', '1000'],
  'О': ['0110', '1001', '1001', '1001', '0110'], 'Г': ['1111', '1000', '1000', '1000', '1000'],
  'Е': ['111', '100', '110', '100', '111'], 'С': ['0111', '1000', '1000', '1000', '0111'],
  'Н': ['1001', '1001', '1111', '1001', '1001'], 'А': ['0110', '1001', '1111', '1001', '1001'],
  'У': ['1001', '1001', '0111', '0001', '1110'], 'К': ['1001', '1010', '1100', '1010', '1001'],
};
...
export function generateBrutalistDusk(config: BrutalistDuskConfig) {
  ...
  const voxels: ConcreteVoxel[] = [], micros = new Map<number, ConcreteVoxel>();
  const structures: Structure[] = [], viaducts: Viaduct[] = [];
  const openings: { x: number; y: number; z: number; w: number; h: number; d: number }[] = [];
  const portals: { x: number; z: number; bottom: number; top: number; w: number; d: number }[] = [];
  ...
  const micro = (x: number, y: number, z: number, color: number, intensity = 0) => {
    ...
    micros.set(key, { x: x + MICRO / 2 - ox - width / 2, y, z: z + MICRO / 2 - oz - depth / 2,
  ...
  for (let rx = Math.floor((ox + 128) / region) - 1; rx <= Math.floor((ox + width + 128) / region) + 1; rx++) {
    for (let rz = Math.floor((oz + 128) / region) - 1; rz <= Math.floor((oz + depth + 128) / region) + 1; rz++) {
      ...
      if (!near({ x: cx - 124, z: cz - 124, w: 248, d: 248 })) continue;
      ...
      lettering(cx - 21, beamY + 8, hall.z + hall.d, 'ПРОГРЕСС', 0.625, C.red);
      ...
      portals.push({ x: left + 33, z: front + 5, w: right - left - 51, d: 10, bottom: lowerY + 7, top: beamY - 4 });
      ...
      lettering(core.x + 3, height - 8, core.z + core.d, 'СССР', 0.5, C.red);
      lettering(right, beamY - 20, pierRight.z + pierRight.d, 'НАУКА', 0.625, C.cream);
      ...
      [...'СССР'].forEach((letter, index) => lettering(core.x + 3.5, scaled(0.37) + 10.5 - index * 3, core.z + core.d + 1, letter, 0.375, C.cream, lights > 0 ? 0.9 : 0));
      ...
      lettering(cx - 8, scaled(0.29) + 1, cz + 12, 'ПРОГРЕСС', 0.375, C.cream, lights > 0 ? 0.7 : 0);
      ...
      while (z < cz + 117) {
        ...
        while (x < cx + 116) {
          ...
          if (!inCore && !inTransit && near(footprint) && h(202) < density) {
      ...
      if (config.brutalTransit > 0) {
        const rail = (x: number, z: number, length: number, level: number, alongX: boolean) => {
          ...
          viaducts.push({ x, z, length, level, alongX, supports });
        };
        ...
        lettering(cx - 44, level - 1.75, railZ + 14, 'ПРОГРЕСС', 0.25, C.cream);
  ...
  for (let y = 0; y < ceiling; y++) for (let z = 0; z < depth; z++) for (let x = 0; x < width; x++) {
    ...
    const { color, emission, weathered } = palette[id];
    const wx = x + ox, wz = z + oz;
    // Correlated pour patches and vertical runoff, plus restrained aggregate variation.
    ...
    voxels.push({ x: x + 0.5 - width / 2, y, z: z + 0.5 - depth / 2, sizeX: 1, sizeY: 1, sizeZ: 1,
      color: weathered && !emission ? tint(color, shade) : color, material: emission ? 'emissive' : 'normal', emissive: emission > 0, intensity: emission });
    standardCount++;
    if (emission) emissiveCount++;
    heights[x + z * width] = Math.max(heights[x + z * width], y + 1);
  }
  for (const v of micros.values()) {
    const x = Math.floor(v.x + width / 2), z = Math.floor(v.z + depth / 2);
    // Discard ornament buried by later structural work (also prevents z-fighting).
    if (get(x, Math.floor(v.y), z)) continue;
    const wx = v.x + ox + width / 2, wz = v.z + oz + depth / 2;
    if (openings.some(o => wx >= o.x && wx < o.x + o.w && wz >= o.z && wz < o.z + o.d && v.y >= o.y && v.y < o.y + o.h)) continue;
    voxels.push(v);
    if (v.emissive) emissiveCount++;
    heights[x + z * width] = Math.max(heights[x + z * width], v.y + MICRO);
  }
  return { voxels, heights, width, depth, standardCount, emissiveCount, structures, viaducts, portals };
}

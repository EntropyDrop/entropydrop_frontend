...
export interface CanyonVoxel {
  x: number; y: number; z: number; sizeX: 1 | 0.125; sizeY: 1 | 0.125; sizeZ: 1 | 0.125;
  color: number; material: 'normal' | 'emissive'; emissive: boolean; intensity: number;
}
...
export function generateTitanCanyon(config: CanyonConfig) {
  ...
  const cells = new Uint16Array(layer * ceiling), heights = new Float32Array(layer), terrain = new Float32Array(layer);
  const palette = [{ color: 0, emission: 0, weather: false }], ids = new Map<string, number>();
  const micros = new Map<number, CanyonVoxel>(), voxels: CanyonVoxel[] = [];
  ...
  const micro = (x: number, y: number, z: number, color: number, emission = 0) => {
    ...
    micros.set(key, { x: x + M / 2 - ox - width / 2, y, z: z + M / 2 - oz - depth / 2,
      sizeX: M, sizeY: M, sizeZ: M, color: activePalette?.get(color) ?? color, emissive: emission > 0, material: emission > 0 ? 'emissive' : 'normal', intensity: emission });
  };
  ...
  const emit = (v: CanyonVoxel) => {
    voxels.push(v);
    const i = Math.floor(v.x + width / 2) + Math.floor(v.z + depth / 2) * width;
    heights[i] = Math.max(heights[i], v.y + v.sizeY);
  };
  ...
  for (let y = 0; y < ceiling; y++) for (let z = 0; z < depth; z++) for (let x = 0; x < width; x++) {
    ...
    const p = palette[id], emissive = p.emission > 0;
    const stain = hash(Math.floor((x + ox) / 3), Math.floor((z + oz) / 3), Math.floor(y / 11) + 300, seed);
    const color = p.weather && !emissive ? tint(p.color, 1 + (stain - 0.5) * 0.16 * config.canyonWear) : p.color;
    emit({ x: x + 0.5 - width / 2, y, z: z + 0.5 - depth / 2, sizeX: 1, sizeY: 1, sizeZ: 1,
      color, emissive, material: emissive ? 'emissive' : 'normal', intensity: p.emission });
    standardCount++; if (emissive) emissiveCount++;
  }
  for (const v of micros.values()) {
    if (get(Math.floor(v.x + width / 2), Math.floor(v.y), Math.floor(v.z + depth / 2))) continue;
    emit(v); if (v.emissive) emissiveCount++;
  }
  const host = visible.filter(s => {
    const p = transform(24, 32, 0, 0, s);
    return s.base + 7 < ceiling && get(Math.floor(p.x - ox), s.base + 3, Math.floor(p.z - oz));
  }).sort((a, b) => Math.hypot(a.x - config.offsetX, a.z - config.offsetZ) - Math.hypot(b.x - config.offsetX, b.z - config.offsetZ))[0];
  const deckView = host ? (() => {
    const p = transform(24, 32, 0, 0, host), q = transform(-80, 87, 0, 0, host);
    return { position: { x: p.x - ox - width / 2, y: host.base + 6.25, z: p.z - oz - depth / 2 },
      target: { x: q.x - ox - width / 2, y: host.base + 15, z: q.z - oz - depth / 2 } };
  })() : null;
  const centered = gears.filter(g => g.center.x > ox + 5 && g.center.x < ox + width - 5
    && g.center.z > oz + 5 && g.center.z < oz + depth - 5 && g.center.y < ceiling);
  const complete = centered.filter(g => g.center.x > ox + 5 && g.center.x < ox + width - 5
    && g.center.z - g.radius > oz && g.center.z + g.radius < oz + depth && g.center.y + g.radius < ceiling);
  const factories = visible.filter(s => s.x > ox + 20 && s.x < ox + width - 20
    && s.z > oz + 40 && s.z < oz + depth - 40 && s.base + 24 < ceiling)
    .map(s => ({ site: s.id, radius: 26, center: { x: s.x - s.bank * 8, y: s.base + 23, z: s.z } }));
  // A clipped-off gear is not a valid camera subject. Fall back to an actual
  // factory in the crop, then to a view of the terrain when industry is absent.
  const landmarks = complete.length ? complete : centered.length ? centered : factories;
  const largest = Math.max(0, ...landmarks.map(g => g.radius));
  const hero = landmarks.filter(g => g.radius >= largest - 2)
    .sort((a, b) => Math.hypot(a.center.x - config.offsetX, a.center.z - config.offsetZ) - Math.hypot(b.center.x - config.offsetX, b.center.z - config.offsetZ))[0];
  const overviewView = hero ? (() => {
    const site = sites.find(s => s.id === hero.site)!;
    const center = { x: hero.center.x - ox - width / 2, y: hero.center.y, z: hero.center.z - oz - depth / 2 };
    return { position: { x: center.x - site.bank * hero.radius * 4.5, y: center.y + hero.radius * 0.65, z: center.z + hero.radius * 2.4 },
      target: { x: center.x, y: center.y - hero.radius * 0.22, z: center.z } };
  })() : { position: { x: width * 0.55, y: Math.max(ceiling, 100), z: depth * 0.6 },
    target: { x: 0, y: Math.min(ceiling * 0.4, 45), z: 0 } };
  return { voxels, heights, terrain, width, depth, sites: visible, gears, cooling, turbines, bridges, standardCount, emissiveCount, deckView, overviewView };
}

...
export interface HarborVoxel extends Point {
...
export function generateColossusHarbor(config: HarborConfig) {
  ...
  const palette = [{ color: 0, emission: 0 }], ids = new Map<string, number>(), micros = new Map<number, HarborVoxel>();
  const voxels: HarborVoxel[] = [], plan = planColossusHarbor(config, 180);
  ...
  const micro = (x: number, y: number, z: number, color: number, emission = 0) => {
    ...
    micros.set(key, { x: x + M / 2 - ox - width / 2, y, z: z + M / 2 - oz - depth / 2,
      sizeX: M, sizeY: M, sizeZ: M, color, material: emission > 0 ? 'emissive' : 'normal', emissive: emission > 0, intensity: emission });
  };
  ...
  const emit = (v: HarborVoxel) => { voxels.push(v); const i = Math.floor(v.x + width / 2) + Math.floor(v.z + depth / 2) * width; heights[i] = Math.max(heights[i], v.y + v.sizeY); };
  ...
  for (let y = 0; y < ceiling; y++) for (let z = 0; z < depth; z++) for (let x = 0; x < width; x++) {
    ...
    const color = emissive || id === waterId ? p.color : tint(p.color, 1 + (stain - 0.5) * 0.26 * config.harborWear);
    emit({ x: x + 0.5 - width / 2, y, z: z + 0.5 - depth / 2, sizeX: 1, sizeY: 1, sizeZ: 1, color, material: emissive ? 'emissive' : 'normal', emissive, intensity: p.emission });
    standardCount++; if (emissive) emissiveCount++;
  }
  for (const v of micros.values()) { if (get(Math.floor(v.x + width / 2), Math.floor(v.y), Math.floor(v.z + depth / 2))) continue; emit(v); if (v.emissive) emissiveCount++; }
  const local = (p: Point) => ({ x: p.x - ox - width / 2, y: p.y, z: p.z - oz - depth / 2 });
  const statues = plan.statues.filter(s => near(s.x, s.z, s.height * 0.85));
  const complete = statues.filter(s => {
    const wingSpan = s.family === 'winged' ? 0.51 * s.spread : s.family === 'eagle' ? 0.55 * s.spread : 0;
    const halfWidth = s.height * Math.max(0.46, wingSpan + 0.09 + Math.abs(s.lean) * 1.2);
    return s.x - halfWidth > ox && s.x + halfWidth < ox + width && s.z - s.height * 0.25 > oz
      && s.z + s.height * 0.4 < oz + depth && s.base + 5 + s.height * 1.25 < ceiling;
  });
  const hero = [...complete].sort((a, b) => b.height - a.height)[0];
  const overviewView = hero ? { position: local({ x: hero.x + 325, y: hero.base + 76, z: hero.z + 280 }), target: local({ x: hero.x + 90, y: hero.base + hero.height * 0.24, z: hero.z + 5 }) }
    : { position: { x: width * 0.52, y: Math.max(95, ceiling * 0.7), z: depth * 0.78 }, target: { x: 0, y: Math.min(ceiling * 0.35, 50), z: 0 } };
  const statueViews = complete.map(s => ({ family: s.family, id: s.id, position: local({ x: s.x + s.height * 0.62, y: s.base + s.height * 0.6, z: s.z + s.height * 1.75 }), target: local({ x: s.x, y: s.base + s.height * 0.53, z: s.z }) }));
  const host = complete[0], deckView = host ? { position: local({ x: host.x + 4, y: host.base + 7.25, z: host.z + Math.ceil(host.height * 0.18) - 2 }), target: local({ x: host.x + 45, y: host.base + 18, z: host.z + 90 }) } : null;
  return { voxels, heights, terrain, width, depth, statues, factories: plan.factories.filter(f => near(f.x, f.z, 45)), buildings, transit, boats, standardCount, emissiveCount, overviewView, statueViews, deckView };
}

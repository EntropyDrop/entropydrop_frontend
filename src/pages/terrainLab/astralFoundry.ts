...
export function generateAstralFoundry(config: FoundryConfig) {
  ...
  const layer = width * depth, cells = new Uint16Array(layer * ceiling), heights = new Float32Array(layer);
  const voxels: FoundryVoxel[] = [], micros = new Map<number, FoundryVoxel>();
  ...
  const micro = (x: number, y: number, z: number, color: number, intensity = 0) => {
    ...
    micros.set(key, { x: x + M / 2 - ox - width / 2, y, z: z + M / 2 - oz - depth / 2,
      sizeX: M, sizeY: M, sizeZ: M, color: materialMap?.get(color) ?? color,
      emissive: intensity > 0, material: intensity > 0 ? 'emissive' : 'normal', intensity });
  };
  ...
  const ring = (x: number, y: number, z: number, radius: number, accent: number, gear: boolean) => {
    ...
    for (const rr of [radius - 0.75, radius - 2.75, radius - 3.125]) {
    ...
    for (let dy = -2; dy <= 2; dy += M) for (let dx = -2; dx <= 2; dx += M) {
  ...
  const emit = (v: FoundryVoxel) => {
    voxels.push(v);
    const index = Math.floor(v.x + width / 2) + Math.floor(v.z + depth / 2) * width;
    heights[index] = Math.max(heights[index], v.y + v.sizeY);
  };
  ...
  for (let y = 0; y < ceiling; y++) for (let z = 0; z < depth; z++) for (let x = 0; x < width; x++) {
    ...
    // Panel-sized variation and vertically correlated oxidation, in world coordinates.
    const stain = hash(Math.floor((x + ox) / 3), Math.floor((z + oz) / 3), Math.floor(y / 17) + 900, seed);
    const grain = hash(x + ox, z + oz, y + 1000, seed);
    const color = p.weather && !emissive ? tint(p.color, 1 + ((stain - 0.5) * 0.10 + (grain - 0.5) * 0.035) * config.forgeWear) : p.color;
  ...
  const host = visible.filter(n => {
    if (!n.major || n.decks[0] + 5 >= ceiling) return false;
    const p = transform(-Math.floor(n.w / 2) + n.w - 1.5, -Math.floor(n.d / 2) + n.d + 2, 0, 0, n);
    return Boolean(get(Math.floor(p.x - ox), n.decks[0] - 1, Math.floor(p.z - oz)));
  })
    .sort((a, b) => Math.hypot(a.x - config.offsetX, a.z - config.offsetZ) - Math.hypot(b.x - config.offsetX, b.z - config.offsetZ))[0];
  const deckView = host ? (() => {
    const z = -Math.floor(host.d / 2) + host.d, x = -Math.floor(host.w / 2);
    const p = transform(x + host.w - 1.5, z + 2, 0, 0, host), q = transform(x - 100, z + 62, 0, 0, host);
    return { position: { x: p.x - ox - width / 2, y: host.decks[0] + 2.25, z: p.z - oz - depth / 2 },
      target: { x: q.x - ox - width / 2, y: host.decks[0] + 18, z: q.z - oz - depth / 2 } };
  })() : null;
  return { voxels, heights, width, depth, nodes: visible, links, machinery, ships, standardCount, emissiveCount, deckView };
}

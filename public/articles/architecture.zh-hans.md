...
公开接口 `/skin/api/open/ledger` 只读取 `paypal`、`aws` 和 `bank` 三类外部 provider，并只展示 `posted` 或 `estimated` 状态的记录。平台内部订单、手工运营记录和未确认同步结果不会进入公开总账视图。后续接入银行 API 时，只需要新增同步适配器，将银行流水写入同一张 `external_ledger_entries` 表，而不需要再为每个 provider 建立独立账本模型。

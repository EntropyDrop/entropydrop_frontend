...
The public endpoint `/skin/api/open/ledger` reads only the `paypal`, `aws`, and `bank` external providers, and only exposes entries with `posted` or `estimated` status. Local orders, manual operating records, and unconfirmed sync results do not appear in the public ledger view. When bank APIs are added later, a new sync adapter can write bank transactions into the same `external_ledger_entries` table without introducing another provider-specific ledger model.
